...
import com.np.spark.data.RandomForestRequest;
...
public class RandomForestKernel extends SparkKernel {

    public static void process(Analysis an, final RandomForestRequest lrr) {
        ...
        try {
            ...
            if (lrr.itsClassification())
                model = RandomForest.trainClassifier(training, lrr.getNumClasses(), lrr.getCategoricalFeaturesInfo(), lrr.getNumTrees(), lrr.getFeatureSubsetStrategy(), lrr.getImpurity(), lrr.getMaxDepth(), lrr.getMaxBins(), seed);
            else
                model = RandomForest.trainRegressor(training, lrr.getCategoricalFeaturesInfo(), lrr.getNumTrees(), lrr.getFeatureSubsetStrategy(), lrr.getImpurity(), lrr.getMaxDepth(), lrr.getMaxBins(), seed);
            ...
            StringBuilder randomForestString = new StringBuilder();
            randomForestString.append("TreeEnsembleModel regressor with " + model.trees().length + " trees\n");
            for (int k=0;k<model.trees().length;k++) {
                String reFormattedDecisionTree = Util.reFormatDecisionTree(model.trees()[k].toDebugString());
                randomForestString.append("********** Tree " + k + "  **********\n");
                randomForestString.append(reFormattedDecisionTree);
                randomForestString.append("\n");
            }
            System.out.println(randomForestString.toString());
            response.setDebugTree(randomForestString.toString());
